...
export default function LoginForm() {
  ...
  // Se mantiene en true desde el click hasta entrar a la app (navegación) o
  // hasta que falle — por eso no se apaga en el camino feliz.
  ...
  return (
    <div className="space-y-4">
      <button
        ...
        disabled={loading}
        className="flex w-full items-center justify-center gap-2.5 rounded-lg border border-linea bg-superficie px-4 py-2.5 font-medium text-tinta transition-colors hover:bg-lienzo focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-azul disabled:cursor-not-allowed disabled:opacity-70"
      >
        {loading ? (
          <LoadingDots className="py-1 text-acero" />
        ) : (
          <>
      ...
      <form onSubmit={handleEmail} className="space-y-3">
        ...
        <button
          type="submit"
          disabled={loading}
          className="flex w-full items-center justify-center rounded-lg bg-azul px-4 py-2.5 font-semibold text-white transition-colors hover:bg-azul-press focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-azul disabled:cursor-not-allowed disabled:opacity-70"
        >
          {loading ? <LoadingDots className="py-1 text-white" /> : isRegister ? 'Crear cuenta' : 'Iniciar sesión'}
        </button>
      </form>
      ...
      <button
        ...
        disabled={loading}
        className="w-full text-center text-sm font-medium text-azul hover:text-azul-press disabled:opacity-50"
      >
