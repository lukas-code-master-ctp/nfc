...
Ver `.env.example`. Las `NEXT_PUBLIC_FIREBASE_*` son públicas (config web). `FIREBASE_PRIVATE_KEY` y `CRON_SECRET` son secretos. En Vercel, Vercel Cron inyecta el `Authorization: Bearer ${CRON_SECRET}` solo. `NEXT_PUBLIC_APP_URL` debe ser la URL de producción (base de los enlaces NFC).
