...
- **Cupo del plan**: `plan.maxVehiculos` limita cuántos vehículos puede crear el usuario. Lógica pura en `lib/plan.ts` (`maxVehiculos`, `planCapacity`); default `DEFAULT_PLAN` (3). Se **enforca en el servidor** (`POST /api/vehicles` responde 409 `plan_limit` al tope) y se muestra en el dashboard (`components/PlanCapacity.tsx` + botón "Nuevo vehículo" deshabilitado). **Lo configura un admin**, no el usuario.
